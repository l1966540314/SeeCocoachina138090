...
#import "JBMoudleRouter.h"
#import <objc/runtime.h>

@interface JBMoudleRouter()
...
@implementation JBMoudleRouter

static JBMoudleRouter *_router;
#pragma mark - 单例
+ (instancetype)router {
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [self loadClassesConfirmToProtocol:@protocol(JBMoudleProtocol)];
    }
    return self;
}
...
#pragma mark - Private
- (Class<JBMoudleProtocol>)findClassForProtocol:(Protocol*)protocol {
    ...
    if (cacheClass) {
        ...
        //未缓存的协议实现类
        Class<JBMoudleProtocol> responseClass = [self assertForMoudleWithProtocol:protocol];
